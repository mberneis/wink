...
class wink
{
    ...
    public function authPassword($clientID, $clientSecret, $username, $password)
    {
        ...
        if ($auth && $auth->timestamp==date("Ym")) {
            ...
            $this->token_endpoint = $auth->token_endpoint;
            $this->getDevices();
            return true;
        }

        // Comment out above for fresh token
        ...
        if (!empty($resp) && empty($resp['errors'])) {
            $resp['data']['timestamp'] = date("Ym");
            file_put_contents("/tmp/nimbus_{$username}.json", json_encode($resp['data']));
